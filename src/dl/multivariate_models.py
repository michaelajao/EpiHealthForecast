...
class SingleStepRNNModel(BaseModel):
    ...
    def forward(self, batch: Tuple[torch.Tensor, torch.Tensor]):
        ...
        # Pass the extracted timestep through the fully connected layer
        x = self.fc(x_last_timestep)
...
class Seq2SeqwAttnModel(BaseModel):
    ...
    def _build_network(self):
        ...
        self.fc = nn.Linear(
            self.hparams.decoder_params.hidden_size * dec_bi_directional_multiplier, self.hparams.input_size
        )
...
class TransformerModel(BaseModel):
    ...
    def _build_network(self):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(100, self.hparams.input_size * self.hparams.multi_step_horizon),
)
