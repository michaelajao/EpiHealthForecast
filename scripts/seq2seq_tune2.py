...
def neighbor(params, param_bounds):
    ...
    # Ensure valid combination of encoder_type and decoder_type
    while True:
        encoder_type = np.random.choice(param_bounds["encoder_type"])
        decoder_type = np.random.choice(param_bounds["decoder_type"])
        if not (
            (encoder_type == "LSTM" and decoder_type == "GRU") or 
            (encoder_type == "GRU" and decoder_type == "LSTM") or 
            (encoder_type == "LSTM" and decoder_type == "RNN") or 
            (encoder_type == "RNN" and decoder_type == "LSTM")
        ):
            break
...
def main():
    ...
    param_bounds = {
        ...
        "decoder_type": ["GRU", "LSTM", "RNN"],
        "hidden_size": (32, 256), 
        "num_layers": (1, 10),
        "learning_rate": (1e-6, 1e-2)
    }

    initial_params = ["GRU", "RNN", 32, 1, 1e-2]
    initial_temp = 10000
    steps = 500
    ...
    plt.savefig(source_data/"images/optimization/temperature_decay_plot3.png")
    plt.show()
    ...
    plt.savefig(source_data/"images/optimization/convergence_plot3.png")
    plt.show()
